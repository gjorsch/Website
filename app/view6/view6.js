...
.controller('view6Ctrl', ["$scope", "$http",'$location', '$anchorScroll', function($scope, $http, $location, $anchorScroll) {
  $scope.language = {
    selected: "en",
  ...
  $scope.updatetree  = function(){
    $http.get("data/skos_resources.json").success(function(data){
      lang = $scope.language.selected.short;
      console.log("click");
      console.log($scope.language.selected.short);
      data = data["@graph"];
  ...
  $http.get("data/output_export_skos-xl_resources.rdf.json").success(function(links){
    $scope.links = {
      repeatSelect: null,
